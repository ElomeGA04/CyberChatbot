...
Asks the user for their name and uses it throughtout the conversation.
...
Enhanced console UI:
Uses colored text, decorative borders, section haders, and dividers for a structured interface.

Typing effect:
Similates a conversational feel with slight delays when the bot types bot types its responses.
...
The presentation demonsrates:
1. Code structure and class relationships between Program.cs, ResponseHandler.cs, and AudioPlayer.cs
1. Voice greeting integration using System.Media.SoundPlayer
1. Console UI formatting techniques including colored text, borders, and typing effect
1. Input validation logic for empty entries and unsupported queries
1. CI pipeline setup and successful run using Github Actions
1. Application functionality showcasing the chatbot's responses to various cybersecurity topics
